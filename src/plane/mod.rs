...
impl Plane {
    pub fn new<F: Facade>(rad: f32, w: u32, h: u32, display: &F) -> Plane {
        let segments = 50;
        ...
        let selector_v_buffer = VertexBuffer::new(display, &selector_vertices)
                                .unwrap();
        ...
        let selector_indices = IndexBuffer::new(display,
        ...
        Plane {
            ...
            mouse_pos: Vector2::new(0.0, 0.0), mouse_offset: Vector2::new(0.0, 0.0), button_rad: rad,
    ...
    pub fn draw<S: Surface>(&self, target: &mut S) {
        ...
        // Make the draw call
        target.draw(&self.selector_v_buffer, 
    ...
    pub fn handle_mouse_click (
        ...
        self.state = match (self.state, button_state, button) {
            (PlaneState::Rest, ElementState::Pressed, MouseButton::Left) => {
                ...
                println!("mouse_offset: {:?}, button_pos: {:?}, mouse_pos: {:?}", self.mouse_offset, self.button_pos, self.mouse_pos);
            ...
            (PlaneState::Drag, ElementState::Released, MouseButton::Left) => {
                PlaneState::Rest
            },
            (state, button_state, button) => {
                println!("ERROR: unexpected transiton for plane, {:?} -> ({:?}, {:?})",
                    state, button_state, button);
                self.state
            },
        }
